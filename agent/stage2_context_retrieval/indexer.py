...
def build_index(playwright_root: Path, knowledge_dir: Path, vector_store_dir: Path) -> list[CodeChunk]:
    chunks = chunk_playwright_repo(playwright_root, knowledge_dir)
    vector_store_dir.mkdir(parents=True, exist_ok=True)
    _write_json_index(chunks, vector_store_dir)
...
def _write_json_index(chunks: list[CodeChunk], vector_store_dir: Path) -> None:
    path = vector_store_dir / INDEX_FILE
    path.write_text(
